...
* `luaD_reallocstack` 调用`luaM_reallocvector`去分配空间，修改`lua_State`相关成员
...
```
当处于代码块，`UpVal::v`指向数据栈上的对象，`UpVal::u::l`为串联`UpValue`的指针。当离开代码块，栈上对象被销毁，`UpVal::u::value`保存原数据栈上的对象，`UpVal::v`指向`UpVal::u::value`
...
`GCObject *lua_State::openupval` 当前线程的`open upvalues`单向链表，`UpVal global_State::uvhead` 全部`open upvalues`的双向链表

* `luaF_findupval` 在查找`openupval`查找`UpValue`，找到则返回，否则创建并插入`openupval`和`uvhead`
* `luaF_close` 删掉无人引用的`UpValue`、把数据从数据栈上复制到`UpVal::u::value`结构，并修正`UpVal::v`


## 异常
为了能够实现嵌套的异常，封装了`::jmp_buf`。由`lua_longjmp *lua_longjmp::previous`串起了异常链
```
struct lua_longjmp {
...
* `luaD_rawrunprotected` 向异常链插入新`lua_longjmp`恢复点（修改`lua_State::errorJmp`）、调用`LUAI_TRY`，删除`lua_longjmp`恢复点（恢复`lua_State::errorJmp`）
